...
<div class="modal-body">
	...
	<table class="table table-condensed">
		<tr><td>Modalidad: </td><td> <select class="form-control" name="modename" style="width:100%;"><?php foreach ($mc_modes as $k=>$v){echo '<option value="'.$v['id'].'">'.$v['modename'].'</option>';} ?></select></td></tr>
		<tr><td>Permiso: </td><td><input type="text" class="form-control" name="nodeperm"/></td></tr>
		<tr><td colspan="2" class="text-right"><button class="btn btn-default" onclick="javascript:addextracmd();">Añadir</button></tr>
	</table>
	<hr/>
	<label>Permisos extra actuales</label>
	<table id="extracmdtbl" class="table table-striped table-hover table-condensed">
		<thead>
			<tr><th>Modalidad</th><th>Permiso</th><th>Retirar?</th></tr>
		</thead>
		<tbody>
			<?php
...
<script type="text/javascript">
	...
	function delextracmd(id){
		$.ajax({
			...
			success: function (data, textStatus, jqXHR) {
				console.log(data);
				$("#extraperm-"+id).fadeOut("slow",function(){$(this).remove();});
